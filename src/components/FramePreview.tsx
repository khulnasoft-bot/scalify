...
export const FramePreview: React.FC<FramePreviewProps> = ({ frameId, code, width, height }) => {
  ...
  const { theme, setHoveredElement, setSelectedElement, selectedElement, updateFrame } = useCanvasStore();
      ...
      (function() {
        ...
        window.addEventListener('message', function(event) {
          ...
          if (event.data.type === 'SYNC_SELECTION') {
            const selectedElementId = event.data.payload.selectedElementId;
            document.querySelectorAll('.stitch-selected-highlight').forEach(el => {
              el.classList.remove('stitch-selected-highlight');
            });
            if (selectedElementId) {
              const target = document.querySelector('[data-stitch-id="' + selectedElementId + '"]');
              if (target) {
                target.classList.add('stitch-selected-highlight');
              }
            }
  ...
  useEffect(() => {
    ...
    const sendSyncMessage = () => {
      const selectedId = (selectedElement && selectedElement.frameId === frameId) 
        ? selectedElement.elementId 
        : null;
        
      iframe.contentWindow?.postMessage({
        type: 'SYNC_SELECTION',
        payload: { selectedElementId: selectedId }
      }, '*');
    };
