    ...
    <div className="border-t border-slate-100 bg-white/95 backdrop-blur-md p-4 shrink-0 shadow-lg z-20">
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <form onSubmit={handleGenerate} className="flex gap-2 items-end">
          {/* Main prompt input */}
          <div className="flex-1 relative flex items-end bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 shadow-inner">
            <Sparkles className="w-4 h-4 text-indigo-600 mr-2 mb-1.5 shrink-0" />
            <textarea
              ...
              className="w-full bg-transparent border-0 text-slate-800 text-xs focus:ring-0 focus:outline-none placeholder-slate-400 font-medium py-1 resize-none min-h-[24px]"
            ...
            <div className="flex items-center gap-1.5 shrink-0 mb-0.5 ml-2">
              ...
              {prompt.trim() && (
                <button
                  ...
                  className="text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 text-white shadow-sm flex items-center gap-1 transition-all active:scale-95 cursor-pointer"
                  title="Transform prompt into a polished layout prompt (adds visual layout keywords & structured sections)"
                >
                  {isEnhancing ? (
